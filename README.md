...
- **两种估值模式**：营收驱动 FCFF（A 股自动取年报营收）/ 净利驱动 FCFE（A/港/美通用，市值÷PE 反推净利）
  - **净现金口径正确性**：「+净现金」桥接只用于 FCFF/企业价值路径；FCFE 折现所得本身已是股权价值，**净负债不二次扣减**（债务利息已在盈利里，否则像可口可乐会被重复多砍一个净负债），仅把正的超额现金加一次
- **多市场 + 货币自洽**：A 股 ¥ / 港股 HK$ / 美股 US$
- **实时抓取**：东方财富为主、腾讯行情为备用源（JSONP 绕过 CORS）
- **机构一致预期自动填充增速**：抓取股票后，自动用东财卖方一致预期（近六月平均）填好未来各预测年的增速、其后线性衰减到永续增速，无需手填。**按模式取对应口径**：营收驱动（详细）填**营收**增速、净利驱动填**净利**增速；切换模式自动重填（A 股有覆盖；港股/美股暂回退为手填）
...
## 一致预期接口（`api/forecast.js`）

无需任何 API key。服务端代理东方财富 F10「盈利预测」（emweb 接口无 CORS/JSONP，故由后端转发并加 CORS），取「近六月平均」机构一致预期 `yctj_chart`，**同时返回营收与净利两套逐年同比增速**（直接用东财自带同比，单位无关、规避基数口径错配；夹逼 [-40%,80%] 防个别毛刺）。前端按当前模式选用——营收驱动填营收增速、净利驱动填净利增速——自动填满 10 年增速网格。**仅 A 股有覆盖**；港股/美股返回 `available:false`，前端回退为默认两段/手填。纯前端（GitHub Pages / 本地双击）无此后端时同样静默回退。
