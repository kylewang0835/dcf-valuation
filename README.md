...
- **机构一致预期自动填充增速**：抓取股票后，自动用东财卖方一致预期（近六月平均净利预测）填好未来各预测年的增速，其后线性衰减到永续增速，无需手填（A 股有覆盖；港股/美股暂回退为手填）
...
无需任何 API key。服务端代理东方财富 F10「盈利预测」（emweb 接口无 CORS/JSONP，故由后端转发并加 CORS），取「近六月平均」机构一致预期的按年归母净利，用前端传来的真实 TTM/年报基数锚定出逐年增速（夹逼防个别口径毛刺）。前端抓取股票时自动调用、自动填充 10 年增速网格。**仅 A 股有覆盖**；港股/美股返回 `available:false`，前端回退为默认两段/手填。纯前端（GitHub Pages / 本地双击）无此后端时同样静默回退。
